...
def isValidValuesString(values_string,target_columns,values_list):
	...
	#print "[isValidValuesString][lexer_list][newSplit2] : "
	#print lexer_list
...
def executeInsert(target_columns,values_list):
	...
	for i in range(0, len(target_schema) ):
		...
		#print "[executeInsert] Load new_bt SUCCESS:" + "scripts\\indexes\\" + str(target_columns[i][0]).lower() + "_" + str(target_columns[i][1]).upper() + "_index_bt.pkl"
		
		index_bt.append(new_bt)
		#print "[executeInsert] Append to index_bt list SUCCESS"
		
		for j in range(0, len(target_columns)):
			#print " Compare: " + str(target_schema[i][1]) + " / " + str(target_columns[j][1])
			if target_schema[i][1] == target_columns[j][1]:
				...
				if index_bt[i][values_list[j]] is None:
					...
					#print " SOMECHECK: " + str(values_list[j]) + "//" + str(values_list[i])
					#print index_bt[i][values_list[j]]
				else:
					error += "\n[ERROR][executeInsert] Duplicate key detected : " + str(index_bt[i])
					#print "[executeInsert] Existing index bt node found"
					#print values_list[j]
					#print index_bt[i][values_list[j]]
				...
				#print "[executeInsert] Save new_bt SUCCESS:" + str(target_schema[i][0]).lower() + "_" + str(target_schema[i][1]).upper() + "_index_bt.pkl"
...
def isValidSQL(input_sql):
	...
	lexer_list = newSplit(tier1_list[0])
	...
	if checkType == "INSERT":
		...
		if isValidTables(tables_string, selected_tables):
			if skipColumnCheck or isValidColumns(columns_string, selected_tables, selected_columns):
				if skipConditionsCheck or isValidConditions(conditions_string, selected_tables):
					...
					outputList.append(conditions_string)
					
					print "SELECT SHIIIIIIIIIIIIIIT - start"
					print selected_tables
					print selected_columns
					print conditions_string
					print "SELECT SHIIIIIIIIIIIIIIT - end"
